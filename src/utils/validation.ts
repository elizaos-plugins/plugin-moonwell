...
export const SUPPORTED_ASSETS: Record<SupportedAsset, { symbol: string; decimals: number; address: string }> = {
    USDC: {
        symbol: "USDC",
        decimals: 6,
...
export function validateAsset(asset: string): string {
    ...
    if (!SUPPORTED_ASSETS[normalizedAsset]) {
        throw createError(
...
export function validateHealthFactor(
    ...
    if (healthFactor < threshold) {
        ...
        if (healthFactor < 1.0) {
            ...
            suggestions.push("WARNING: Your position is close to liquidation threshold");
...
export function validateBorrowCapacity(
    ...
    if (requestedAmount.gt(availableCapacity)) {
        throw createError(
            ...
            { requested: requestedAmount.toString(), available: availableCapacity.toString() },
...
export function validateLiquidity(
    ...
    if (requestedAmount.gt(availableLiquidity)) {
        throw createError(
            ...
            { requested: requestedAmount.toString(), available: availableLiquidity.toString() },
...
export function validateWalletBalance(
    ...
    if (requiredAmount.gt(walletBalance)) {
        throw createError(
            ...
            { required: requiredAmount.toString(), available: walletBalance.toString() },
...
export function parseAmount(amount: string | number, decimals: number): BigNumber {
