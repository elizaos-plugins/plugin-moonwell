...
export class MoonwellService extends Service {
    ...
    constructor(protected runtime: IAgentRuntime) {
        ...
        this.moonwellConfig = {
            network: (runtime.getSetting("MOONWELL_NETWORK") as "base" | "base-sepolia") || "base",
    ...
    static async stop(runtime: IAgentRuntime): Promise<void> {
        ...
        const service = runtime.getService(MoonwellService.serviceType) as MoonwellService;
        if (service) {
            await service.stop();
        }
    }
    ...
    async supply(params: SupplyParams): Promise<SupplyResult> {
        try {
            ...
            if (params.enableAsCollateral) {
                const enterMarketsTx = await this.comptroller!.enterMarkets([await market.getAddress()]);
    ...
    async borrow(params: BorrowParams): Promise<BorrowResult> {
        try {
            ...
            validateHealthFactor(positionAfter.healthFactor, this.moonwellConfig.healthFactorThreshold);
    ...
    async repay(params: RepayParams): Promise<RepayResult> {
        try {
            ...
            const borrowBalance = await market.borrowBalanceStored(this.state.userAddress);
            ...
            // Get updated position
            const positionAfter = await this.getUserPosition();
            const remainingBorrow = await market.borrowBalanceStored(this.state.userAddress);
    ...
    async withdraw(params: WithdrawParams): Promise<WithdrawResult> {
        try {
            ...
            validateHealthFactor(simulatedHealthFactor, this.moonwellConfig.healthFactorThreshold);
    ...
    async getUserPosition(): Promise<UserPosition> {
        try {
            ...
            const [error, collateral, shortfall] = await this.comptroller!.getAccountLiquidity(
    ...
    async getMarketData(asset?: string): Promise<MarketData[]> {
        try {
            ...
            for (const assetSymbol of assetsToCheck) {
                ...
                const marketAddress = await market.getAddress();
                const assetInfo = SUPPORTED_ASSETS[assetSymbol];
                ...
                const collateralFactor = new BigNumber(marketInfo[1].toString()).dividedBy(
                    new BigNumber(10).pow(18)
                );
