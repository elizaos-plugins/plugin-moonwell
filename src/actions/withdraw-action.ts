...
import { validateAmount, validateAsset, SUPPORTED_ASSETS, formatAmount, formatUSD } from "../utils/validation";
...
export const withdrawAction: Action = {
    ...
    validate: async (
        ...
        try {
            ...
            const withdrawKeywords = ["withdraw", "redeem", "remove", "take out", "withdrawal"];
            const hasWithdrawKeyword = withdrawKeywords.some((keyword) => text.includes(keyword));
            ...
            const isGenericWithdrawal = withdrawKeywords.some((keyword) => text.includes(keyword)) &&
                                       !text.includes("aave") && !text.includes("compound");
    ...
    handler: async (
        ...
        try {
            ...
            const supplyPosition = position.supplies.find(s => s.asset === params.asset);
            ...
            if (params.isMax || (params.amount && params.amount.gte(supplyPosition.balance))) {
                withdrawAmount = supplyPosition.balance;
                params.isMax = true;
            }
            
            const formattedWithdrawAmount = formatAmount(withdrawAmount, assetInfo.decimals);
            ...
            if (!position.totalBorrowed.isZero()) {
                ...
                if (estimatedHealthFactor < 1.2) {
                    const safeWithdrawAmount = position.totalSupplied
                        .minus(position.totalBorrowed.multipliedBy(1.5).dividedBy(position.liquidationThreshold));
            ...
            // Format response
            const formattedSupplyBefore = formatAmount(supplyPosition.balance, assetInfo.decimals);
            const formattedSupplyAfter = formatAmount(result.remainingSupply, assetInfo.decimals);
...
function parseWithdrawParams(text: string): WithdrawParams {
    ...
    if (!asset) {
        throw new Error("Could not determine which asset to withdraw. Please specify like 'withdraw 500 USDC' or 'withdraw all DAI'");
    }
    ...
    if (!amount) {
        throw new Error("Could not parse withdrawal amount. Please specify like '500 USDC' or use 'all' for complete withdrawal");
    }
