...
export const supplyAction: Action = {
    ...
    validate: async (
        ...
        try {
            ...
            const supplyKeywords = ["supply", "lend", "deposit", "add collateral", "stake"];
            ...
            const isGenericLending = supplyKeywords.some((keyword) => text.includes(keyword)) &&
...
function parseSupplyParams(text: string): SupplyParams {
    ...
    enableAsCollateral = text.toLowerCase().includes("collateral") || 
    ...
    if (!amount || !asset) {
        throw new Error("Could not parse amount and asset from message. Please specify like '1000 USDC' or '0.5 ETH'");
    }
