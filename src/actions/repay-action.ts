...
export const repayAction: Action = {
    ...
    validate: async (
        ...
        try {
            ...
            const repayKeywords = ["repay", "pay back", "payback", "repay loan", "repay debt", "pay off"];
            ...
            const isGenericRepayment = repayKeywords.some((keyword) => text.includes(keyword)) &&
    ...
    handler: async (
        ...
        try {
            ...
            const debtPosition = position.borrows.find(b => b.asset === params.asset);
            ...
            if (params.isMax || (params.amount && params.amount.gte(debtPosition.balance))) {
            ...
            const formattedRepayAmount = formatAmount(repayAmount, assetInfo.decimals);
            ...
            // Format response
            const formattedDebtBefore = formatAmount(debtPosition.balance, assetInfo.decimals);
            const formattedDebtAfter = formatAmount(result.remainingDebt, assetInfo.decimals);
...
function parseRepayParams(text: string): RepayParams {
    ...
    if (!asset) {
        throw new Error("Could not determine which asset to repay. Please specify like 'repay 300 USDC' or 'repay all DAI debt'");
    }
    ...
    if (!amount) {
        throw new Error("Could not parse repayment amount. Please specify like '300 USDC' or use 'all' for full repayment");
    }
