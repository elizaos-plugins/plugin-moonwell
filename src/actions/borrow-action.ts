...
import { validateAmount, validateAsset, SUPPORTED_ASSETS, formatAmount, formatAPY, formatUSD } from "../utils/validation";
...
export const borrowAction: Action = {
    ...
    validate: async (
        ...
        try {
            ...
            const hasBorrowKeyword = borrowKeywords.some((keyword) => text.includes(keyword));
            ...
            const isGenericBorrowing = borrowKeywords.some((keyword) => text.includes(keyword)) &&
    ...
    handler: async (
        ...
        try {
            ...
            const borrowAmountFormatted = formatAmount(params.amount, assetInfo.decimals);
            ...
            const healthFactorWarning = result.healthFactor < 2.0 
...
function parseBorrowParams(text: string): BorrowParams {
    ...
    if (!amount || !asset) {
        throw new Error("Could not parse amount and asset from message. Please specify like '500 USDC' or '0.1 ETH'");
    }
